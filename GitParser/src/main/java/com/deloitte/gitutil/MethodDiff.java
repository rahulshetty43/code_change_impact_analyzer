...
public class MethodDiff {
    ...
    public ArrayList<ChangeSet> methodDiffInClass(String file1, String file2) throws FileNotFoundException {
        ArrayList<String> changedMethods = new ArrayList();
        ...
        CompilationUnit cu = JavaParser.parse(new File(file1));
//        String packageName 
        Optional<PackageDeclaration> pd = cu.getPackageDeclaration();
        String packageName = pd.get().getNameAsString();
        
//        String[] so = changedMethods.get(0).toString().split("\\.", 2);
//        System.out.println(changedMethods.get(0));
        System.out.println(file1);
        String className = changedMethods.get(0).split("\\.", 2)[0];
        String projectName = null; 
        String tempProject = file2.split("HeadRev/", 2)[1];
//        String projectName = null;
        ArrayList<ChangeSet> changeSet = new ArrayList<ChangeSet>();
        for (String s: changedMethods) {
        	projectName = tempProject.split("/", 2)[0];
        	ChangeSet cs = new ChangeSet(className+".java", projectName, packageName, null);
//        	String[] methodName = new String[2];
//        	System.out.println(s + "  Haha");
        	cs.setMethodName(s.split("\\.", 2)[1]);
        	changeSet.add(cs);
